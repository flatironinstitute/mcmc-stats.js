...
describe("R-Hat", () => {
  test("too few samples", () => {
    const chains = [[1, 2]];
    const actual = potential_scale_reduction(chains);
  ...
  test("basic", () => {
    ...
    // https://github.com/stan-dev/stan/blob/v2.35.0/src/test/unit/analyze/mcmc/potential_scale_reduction_test.cpp#L23-L60
    const expected_rhat = [
  ...
  test("split", () => {
    ...
    for (const [i, expected] of expected_rhat.entries()) {
      const chains = [data1[i + 4], data2[i + 4]];
      const actual = split_potential_scale_reduction(chains);
