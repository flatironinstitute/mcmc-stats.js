...
describe("ESS", () => {
  test("too few samples", () => {
    const chains = [[1, 2]];
    const actual = effective_sample_size(chains);
  ...
  test("one chain", () => {
    ...
    // https://github.com/stan-dev/stan/blob/v2.35.0/src/test/unit/analyze/mcmc/effective_sample_size_test.cpp#L22-L57
  ...
  test("two chains", () => {
    // Based on the unit test in Stan 2.35 but with more digits of precision
    // https://github.com/stan-dev/stan/blob/v2.35.0/src/test/unit/analyze/mcmc/effective_sample_size_test.cpp#L59-L96
  ...
  test("two chains (split)", () => {
    // https://github.com/stan-dev/stan/blob/v2.35.0/src/test/unit/analyze/mcmc/effective_sample_size_test.cpp#L170-L208
    const expected_ess = [
    ...
    for (const [i, expected] of expected_ess.entries()) {
      const chains = [data1[i + 4], data2[i + 4]];
      const actual = split_effective_sample_size(chains);
...
describe("MCSE", () => {
  test("MCSE from ESS", () => {
    ...
    const MCSE = (d: number[][]) => {
      const ess = effective_sample_size(d);
      const std = std_deviation(d.flat());
